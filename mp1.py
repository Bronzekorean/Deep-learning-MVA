...
def generate_noise(nb_samples, free_location=False):
    # Getting im_size:
    im_size = 72**2
    X = np.zeros([nb_samples,im_size])
    ...
    for i in range(nb_samples):
        ...
        category = np.random.randint(3)
        if category == 0:
            X[i], Y[i] = generate_a_rectangle(noise, free_location, denoising = True)
        elif category == 1: 
            X[i], Y[i] = generate_a_disk(noise, free_location, denoising = True)
        else:
            output = generate_a_triangle(noise, free_location, denoising = True)
            X[i], Y[i] = output[0]
    X = (X) / (255)
    Y = (Y)/(255)
    return [X, Y]
...
def generate_test_noise():
    np.random.seed(42)
    [X_test, Y_test] = generate_noise(300, 20)
    return [X_test, Y_test] 
